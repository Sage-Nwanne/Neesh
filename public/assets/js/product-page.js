    (function () {
      ...
      document.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowLeft') { e.preventDefault(); showIndex(current - 1); }
        if (e.key === 'ArrowRight') { e.preventDefault(); showIndex(current + 1); }
      });
...
function saveCart(cart){
  ...
  function addToCart(product, qty){
    ...
    if(idx > -1){
      ...
      cart.items.push({ id: product.id, title: product.title, price_cents: product.price_cents, qty: qty, img: product.img });
    }
    saveCart(cart);
    renderCart();
    openCart();
  ...
  function renderCart(){
    ...
    if(!cart.items.length){
      container.innerHTML = '<div style="padding:14px;color:#666;">Your cart is empty.</div>';
    } else {
      cart.items.forEach(item => {
        ...
        el.innerHTML = `
          <img src="${escapeHtml(item.img || '')}" alt="${escapeHtml(item.title)}" style="width:62px;height:62px;object-fit:cover;border-radius:6px;background:#f6f6f6;">
          <div style="flex:1;min-width:0;">
            ...
            <div class="cart-actions" style="display:flex;align-items:center;gap:6px;margin-top:10px;border-bottom:1px solid #ddd;padding-bottom:15px;">
              ...
              <div style="min-width:34px;text-align:center;font-weight:700;">${item.qty}</div>
          ...
          <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end;">
             <div class="cart-remove" title="Remove"><img src="image/delete.jpg" alt="delete" data-action="remove" data-id="${escapeHtml(item.id)}"></div>
             <div style="font-size:13px;color:#444;margin-top:6px;">
              <strong>${formatMoney(itemTotalCents)}</strong>
            </div>
          </div>
        `;
        el.querySelector('[data-action="decrease"]').addEventListener('click', ()=> updateItemQty(item.id, item.qty - 1));
        el.querySelector('[data-action="increase"]').addEventListener('click', ()=> updateItemQty(item.id, item.qty + 1));
    ...
    const subtotalEl = document.getElementById('subtotal') || document.querySelector('.cart-subtotal');
  ...
  function updateCartCountUI(cart){
    ...
    const el = document.getElementById('cartCount') || document.querySelector('.cart-count');
    if(el) {
...
window.addEventListener('pageshow', () => renderCart());
  ...
  function openCart(){
    ...
    if(cartOverlay){ cartOverlay.style.opacity = '1'; cartOverlay.style.pointerEvents = 'auto'; cartOverlay.setAttribute('aria-hidden','false'); }
  }
  function closeCart(){
    if(cartDrawer) cartDrawer.style.right = '-420px';
    if(cartOverlay){ cartOverlay.style.opacity = '0'; cartOverlay.style.pointerEvents = 'none'; cartOverlay.setAttribute('aria-hidden','true'); }
  }
  ...
  function buildProductFromNode(node, fallbackIndex){
    const dataEl = node.querySelector?.('.product-data') || document.querySelector('.product-data');
    const data = dataEl?.dataset || {};

    const id = data.productId || `product-${fallbackIndex || Date.now()}-${Math.floor(Math.random()*10000)}`;
    const title = data.productTitle || node.querySelector?.('.product_title, .product-title, h1, h2')?.textContent?.trim() || 'Untitled Product';
    const price_cents = toCents(data.productPrice || node.querySelector?.('.my_margin_price, .product-price, .price')?.textContent || 0);
  ...
  const buttons = Array.from(document.querySelectorAll('[data-add-to-cart], .add-to-cart'));
  buttons.forEach((btn, idx) => {
    if (btn.dataset.bound === 'true') return;

    const card = btn.closest('.product, .product-card, .product-item, .product-wrap, .right_container') || document.body;
    ...
    if (qtyContainer && !qtyContainer.dataset.bound) {
      const qtyDisplay = qtyContainer.querySelector('.quantity-display');
      const dec = qtyContainer.querySelector('.quantity-btn.decrease');
      const inc = qtyContainer.querySelector('.quantity-btn.increase');
      let qty = parseInt(qtyContainer.dataset.quantity || (qtyDisplay && qtyDisplay.textContent) || 1, 10) || 1;
      function setQty(n){
    ...
    btn.addEventListener('click', function(e){
      ...
      if (qd) {
        qty = parseInt(qd.textContent, 10) || parseInt(qc.dataset.quantity, 10) || 1;
      } else {
        const globalQty = document.getElementById('quantity');
        if (globalQty) qty = parseInt(globalQty.textContent || globalQty.value, 10) || 1;
      }
  ...
  function bindProductPageControls(){
    ...
    let quantity = parseInt(quantityDisplay?.textContent || quantityDisplay?.value) || 1;
    function setQuantity(n){
      quantity = Math.max(1, Math.floor(Number(n) || 1));
      if(quantityDisplay){
        if(quantityDisplay.tagName === 'INPUT') quantityDisplay.value = quantity;
  ...
  function watchForNewCards(){
    const container = document.querySelector('.collection-grid, .products-grid, .grid') || document.body;
    try {
  ...
  (function init(){
    ...
    if(!existing || !Array.isArray(existing.items)) saveCart({ items: [] });
    renderCart();
    bindAddButtons();
  ...
  window.__miniCart = { loadCart, saveCart, addToCart, renderCart, clearCart, openCart, closeCart };
})();