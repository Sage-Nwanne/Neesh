(function(){
  ...
  function publisher_validateStep(n){
    ...
    for(const publisher_field of publisher_fields){
      ...
      if(publisher_shouldValidate && !String(publisher_input.value || '').trim()){
        ...
        if(!publisher_form._focusedInvalid){ publisher_input.focus(); publisher_form._focusedInvalid = true; }
      } else {
  ...
  publisher_prevBtn?.addEventListener('click', ()=> { if(publisher_currentStep > 0){ publisher_currentStep--; publisher_showStep(publisher_currentStep); } });
  ...
  publisher_form.addEventListener('keydown', function(publisher_e){
    if(publisher_e.key === 'Enter'){
      ...
      if(publisher_currentStep < publisher_steps.length - 1){
        if(!publisher_validateStep(publisher_currentStep)) return;
        publisher_currentStep++; publisher_showStep(publisher_currentStep);
      } else {
        if(!publisher_validateStep(publisher_currentStep)) return;
        publisher_form.submit();
      }
  ...
  function publisher_populateSummary(){
    ...
    publisher_stepElems.forEach((publisher_stepElem, publisher_idx) => {
      ...
      publisher_controls.forEach(publisher_ctrl => {
        ...
        if(publisher_ctrl.type === 'radio'){
          ...
          publisher_value = publisher_ctrl.checked ? 'Yes' : 'No';
          ...
          if(publisher_selectedFiles.length){
            ...
            const publisher_k = document.createElement('div'); publisher_k.className = 'summary-key'; publisher_k.textContent = publisher_key;
            const publisher_v = document.createElement('div'); publisher_v.className = 'summary-val'; publisher_v.appendChild(publisher_imgRow);
            publisher_item.appendChild(publisher_k); publisher_item.appendChild(publisher_v);
        ...
        publisher_item.className = 'summary-item';
        const publisher_k = document.createElement('div'); publisher_k.className = 'summary-key'; publisher_k.textContent = publisher_key;
        const publisher_v = document.createElement('div'); publisher_v.className = 'summary-val'; publisher_v.textContent = publisher_value;
        publisher_item.appendChild(publisher_k); publisher_item.appendChild(publisher_v);
        publisher_sectionWrapper.appendChild(publisher_item);
      });
    ...
    if(publisher_selectedFiles.length){
      const publisher_item = document.createElement('div');
      publisher_item.className = 'summary-item';
      const publisher_k = document.createElement('div'); publisher_k.className = 'summary-key'; publisher_k.textContent = 'Images';
      const publisher_v = document.createElement('div'); publisher_v.className = 'summary-val';
      publisher_selectedFiles.forEach(publisher_file => {
        const publisher_img = document.createElement('img');
