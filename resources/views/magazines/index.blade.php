  ...
  <div class="upload_topmenu_wrapper">
    <div class="upload_topmenu">
      ...
      <div id="sideMenu" class="side-menu">
        ...
        <a href="#">Dashboard</a>
        <a href="Catelog.html">Catalogue</a>
        <a href="#">Orders</a>
        <a href="#">Messages</a>
        <a href="#">Account</a>
        ...
        <div class="main-viewport" aria-live="polite">
          @foreach($magazine->images as $key => $image)
            <img src="{{ asset('storage/' . $image->image_path) }}"
                 data-index="{{ $key }}"
                 class="main-img"
                 style="{{ $key === 0 ? '' : 'opacity:0;transform:scale(1.02)'}}"
        ...
        <div class="thumbs" role="tablist" aria-label="Thumbnails">
          @foreach($magazine->images as $key => $image)
            <button class="thumb {{ $key === 0 ? 'active' : '' }}" data-index="{{ $key }}">
              <img src="{{ asset('storage/' . $image->image_path) }}" alt="Thumbnail {{ $key + 1 }}">
            </button>
          @endforeach
        </div>
